...
def process_title(page_count, text, prev_title, lang):
    prompt = f"""
...
with tab1:
    ...
    if st.button("Run Extract Words"):
        if not uploaded_file:
            ...
            try:
                # images = convert_from_path(file_path, poppler_path=POPPLER_PATH) # In Windows
        ...
        if not uploaded_file:
            ...
            try:
                with st.spinner("Processing..."):
                    ...
                    logger.info(f"OCR processing took {time.time() - start_time:.2f} seconds")
                    ...
                    with ThreadPoolExecutor(max_workers=worker_nums) as executor:
                        ...
                        for page_count, text in enumerate(ocr_results, start=1):
                            print(page_count)
                            future = executor.submit(process_title, page_count, text, prev_title, lang)
                            futures.append(future)
                            prev_title = future.result()[1]
                        page_results = [future.result() for future in futures]
                    
                    logger.info(f"API processing took {time.time() - start_time:.2f} seconds")

                    # Sort results by page number to maintain order
                    page_results.sort(key=lambda x: x[0])
                    
                    for page_count, title in page_results:
                        char = ['\n', '.']
                        for c in char:
                            title = title.replace(c, ' ')
                        if not title or title.lower() in ["none", "null", "unknown", "không xác định"]:
                            title = title_list[-1] if title_list else "Không Xác Định"
